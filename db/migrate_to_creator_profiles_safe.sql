-- =====================================================
-- SAFE MIGRATION SCRIPT: Restructure Database to Use creator_profiles
-- =====================================================
-- This script will:
-- 1. Add authentication fields to creator_profiles safely
...
-- =====================================================
-- STEP 1: Check and add missing columns to video_posts
-- =====================================================

-- Add isDeleted column if it doesn't exist
ALTER TABLE `video_posts` 
ADD COLUMN IF NOT EXISTS `isDeleted` enum('notDeleted','deleted') DEFAULT 'notDeleted' AFTER `Updated_at`;
...
-- =====================================================
-- STEP 2: Add authentication fields to creator_profiles (without constraints first)
-- =====================================================

-- Add authentication fields to creator_profiles (allow NULL initially)
ALTER TABLE `creator_profiles` 
ADD COLUMN IF NOT EXISTS `Email` varchar(255) NULL AFTER `DisplayName`,
ADD COLUMN IF NOT EXISTS `Password` varchar(255) NULL AFTER `Email`,
ADD COLUMN IF NOT EXISTS `PhoneNumber` varchar(15) NULL AFTER `Password`,
ADD COLUMN IF NOT EXISTS `Gender` varchar(25) NULL DEFAULT 'Not Specified' AFTER `PhoneNumber`,
ADD COLUMN IF NOT EXISTS `Access` varchar(50) NULL DEFAULT 'Granted' AFTER `Gender`,
ADD COLUMN IF NOT EXISTS `Unique_id` varchar(255) NULL AFTER `ProfileID`;

-- =====================================================
-- STEP 3: Migrate data from admin to creator_profiles
-- =====================================================
...
-- =====================================================
-- STEP 4: Clean up data and add constraints
-- =====================================================
...
ADD UNIQUE KEY IF NOT EXISTS `uk_username` (`Username`),
ADD UNIQUE KEY IF NOT EXISTS `uk_unique_id` (`Unique_id`);

-- =====================================================
-- STEP 5: Update video_posts table to use ProfileID instead of AuthorID
-- =====================================================
...
-- =====================================================
-- STEP 6: Update article table to use ProfileID instead of AdminId
-- =====================================================
...
-- =====================================================
-- STEP 7: Update category table to use ProfileID instead of AdminId
-- =====================================================
...
-- =====================================================
-- STEP 8: Update users table to use ProfileID instead of AdminId
-- =====================================================
...
-- =====================================================
-- STEP 9: Update creator_profiles to remove AdminId dependency
-- =====================================================
...
-- Drop existing foreign key constraints (ignore errors if they don't exist)
SET @sql = (SELECT GROUP_CONCAT(
    CONCAT('ALTER TABLE `', TABLE_NAME, '` DROP FOREIGN KEY IF EXISTS `', CONSTRAINT_NAME, '`')
    SEPARATOR '; '
) FROM INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
WHERE REFERENCED_TABLE_NAME = 'admin' AND TABLE_SCHEMA = DATABASE());
...
DROP INDEX IF EXISTS `idx_adminid` ON `article`;
DROP INDEX IF EXISTS `idx_adminid` ON `category`;
DROP INDEX IF EXISTS `idx_adminid` ON `users`;
DROP INDEX IF EXISTS `idx_admin_id` ON `creator_profiles`;

-- Add new indexes
ALTER TABLE `article` ADD INDEX IF NOT EXISTS `idx_profileid` (`ProfileID`);
ALTER TABLE `category` ADD INDEX IF NOT EXISTS `idx_profileid` (`ProfileID`);
ALTER TABLE `video_posts` ADD INDEX IF NOT EXISTS `idx_profileid` (`ProfileID`);
ALTER TABLE `users` ADD INDEX IF NOT EXISTS `idx_profileid` (`ProfileID`);

-- =====================================================
-- STEP 12: Drop the admin table
-- =====================================================
