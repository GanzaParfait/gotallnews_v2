-- =====================================================
-- SHORTS SYSTEM DATABASE TABLES - FIXED VERSION
...
-- 8. User playlists for organizing saved videos
...
-- 9. Short video engagement analytics
...
-- 10. User video interactions summary (for quick queries)
...
-- 11. Insert default playlist for each user
DELIMITER //
...
-- 12. Update video analytics when interactions change (FIXED SYNTAX)
DELIMITER //
...
BEGIN
    ...
    INSERT INTO `user_video_interactions` (`UserID`, `VideoID`, `HasLiked`) 
    VALUES (NEW.UserID, NEW.VideoID, 1)
    ON DUPLICATE KEY UPDATE `HasLiked` = 1, `LastInteraction` = NOW();
END//
DELIMITER ;

-- 13. Update video analytics when likes are removed
DELIMITER //
...
-- 14. Update video analytics when comments are added
DELIMITER //
...
BEGIN
    ...
    INSERT INTO `user_video_interactions` (`UserID`, `VideoID`, `HasCommented`) 
    VALUES (NEW.UserID, NEW.VideoID, 1)
    ON DUPLICATE KEY UPDATE `HasCommented` = 1, `LastInteraction` = NOW();
END//
DELIMITER ;

-- 15. Update video analytics when videos are saved
DELIMITER //
...
BEGIN
    ...
    INSERT INTO `user_video_interactions` (`UserID`, `VideoID`, `HasSaved`) 
    VALUES (NEW.UserID, NEW.VideoID, 1)
...
-- 16. Update video analytics when saves are removed
DELIMITER //
...
-- 17. Update video analytics when shares occur
DELIMITER //
...
BEGIN
    ...
    INSERT INTO `user_video_interactions` (`UserID`, `VideoID`, `HasShared`) 
    VALUES (NEW.UserID, NEW.VideoID, 1)
    ON DUPLICATE KEY UPDATE `HasShared` = 1, `LastInteraction` = NOW();
END//
DELIMITER ;

-- 18. Update video analytics when views are tracked
DELIMITER //
...
CREATE INDEX IF NOT EXISTS `idx_video_posts_videoType_status` ON `video_posts` (`videoType`, `Status`);
CREATE INDEX IF NOT EXISTS `idx_video_posts_created_status` ON `video_posts` (`Created_at`, `Status`);
CREATE INDEX IF NOT EXISTS `idx_video_posts_views_status` ON `video_posts` (`Views`, `Status`);
...
-- 21. Create view for quick shorts analytics
CREATE OR REPLACE VIEW `shorts_analytics_view` AS
SELECT 
    v.VideoID,
    v.Title,
    v.Slug,
...
-- 22. Create view for user interaction summary
CREATE OR REPLACE VIEW `user_interactions_summary` AS
SELECT 
    u.UserID,
    u.Username,
    u.DisplayName,
...
-- 29. Show indexes for performance verification
...
-- =====================================================
-- END OF SHORTS SYSTEM DATABASE TABLES - FIXED VERSION
-- =====================================================
